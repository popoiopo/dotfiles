...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Roam
  (setq roamnotes-path (concat surfdrive-dir "roam_notes/"))
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun bc/capitalize-word-at-point ()
    (interactive)
    (let* (($bounds (bc/word-boundary-at-point-or-region))
	   ($p1 (car $bounds))
	   ($p2 (cadr $bounds)))
      (upcase-initials-region $p1 $p2)))

  (defun bc/word-boundary-at-point-or-region (&optional callback)
  ...
  (defun bc/my-kill-thing-at-point (thing)
    "Kill the `thing-at-point' for the specified kind of THING. Found from     https://stackoverflow.com/questions/33442027/how-to-deleteor-kill-the-current-word-in-emacs"
  ...
  (defun bc/capitalize-word-at-point ()
    (interactive)
    (bc/word-boundary-at-point-or-region #'upcase-initials-region))
  (defun bc/downcase-word-at-point ()
    (interactive)
    (bc/word-boundary-at-point-or-region #'downcase-region))
  (defun bc/upcase-word-at-point ()
    (interactive)
    (bc/word-boundary-at-point-or-region #'upcase-region))
  (defun bc/my-kill-word-at-point ()
    "Kill the word at point."
    (interactive)
    (bc/my-kill-thing-at-point 'word))
  (defun bc/capitalize-region (p1 p2)
    (downcase-region p1 p2)
    (upcase-initials-region p1 p2))
  (defun bc/capitalize-fl-word-at-point ()
    "Capitalizes first letter of word and downcases rest"
    (interactive)
    (bc/word-boundary-at-point-or-region #'bc/capitalize-region))

  (global-set-key (kbd "M-C-c") #'bc/capitalize-word-at-point)
  (global-set-key (kbd "C-S-c") #'bc/capitalize-fl-word-at-point)
  (global-set-key (kbd "M-u") #'bc/upcase-word-at-point)
  (global-set-key (kbd "M-l") #'bc/downcase-word-at-point)
  (global-set-key (kbd "M-C-d") #'bc/my-kill-word-at-point)
#+END_SRC
*** Umlaut toggle
Toggles the meta button so Mac can also use umlauts and such
#+BEGIN_SRC emacs-lisp
  (defun bc/umlauts-on()
  ...
  (defun bc/umlauts-off()
    "Disable umlauts with opt keys"
    (interactive)
    (setq mac-option-modifier 'meta)
    )

  (defun bc/toggle-umlauts()
    "Toggle Mac meta key for use of umlauts"
    (interactive)
      ;; use a property “state”. Value is t or nil
    (if (get 'bc/toggle-umlauts 'state)
	(progn
	  (bc/umlauts-on)
	  (put 'bc/toggle-umlauts 'state nil))
      (progn
	(bc/umlauts-off)
	(put 'bc/toggle-umlauts 'state t))))
#+END_SRC
** Custom keystrokes
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Hugo shorts
  (define-key z-map (kbd "h i") 'easy-hugo-image)
  (define-key z-map (kbd "h n") 'easy-hugo-newpost)
  (define-key z-map (kbd "h p") 'easy-hugo-preview)
  (define-key z-map (kbd "h e") 'easy-hugo)
  (define-key z-map (kbd "h g") 'easy-hugo-publish-clever)
  (define-key z-map (kbd "h s") 'easy-hugo-ag)
  ...
  ;; Toggle Meta button
  (define-key z-map (kbd "m") 'bc/toggle-umlauts)
...
#+END_SRC
** Auto-compile
This package provides two minor modes which automatically recompile Emacs Lisp source files. Together these modes guarantee that Emacs never loads outdated byte code files. Hopefully will fix org roam.
#+BEGIN_SRC emacs-lisp
...
*** Twitter bootstrap
#+BEGIN_SRC emacs-lisp
  ;; (load-file "~/.emacs.d/elpa/ox-twbs-20200628.1949/ox-twbs.el")
  (use-package ox-twbs
      :defer t
      :after org)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ;; (load-file "~/.emacs.d/elpa/ox-reveal-20201211.1518/ox-reveal.el")
...
#+BEGIN_SRC emacs-lisp
  ...
  :init
  (setq easy-hugo-basedir "~/github/blog/")
  (setq easy-hugo-url "https://popoiopo.github.io")
  (setq easy-hugo-previewtime "300")
  (setq easy-hugo-default-ext ".org")
  :bind ("C-c C-e" . easy-hugo))

  (defface easy-hugo-help-face
  '((((class color) (background light)) (:bold t :foreground "#FF12FF" :background "#FB4328"))
  (((class color) (background dark)) (:bold t :foreground "#00F2CC" :background "#ABCDEF")))
  ""
...
#+BEGIN_SRC emacs-lisp
  (setq org-todo-keyword-faces
	'(
	  ...
	  ("TEACHING" . (:foreground "#a768f9" :weight bold :box (:line-width 2 :style released-button)))
  ...
  (setq org-todo-keywords
	'((sequence "TODO(t/!)" "DOING(D/!)" "NEXT(n/!)" "DELEGATED(o)" "WAITING(w@/!)" "FLEETING(f)" "TEACHING(T)" "PROJ(p)" "MEETING(m)" "LONGTERM(l@/!)" "REPEAT(r)" "|" "CANCELED(c@/!)" "DONE(d/!)")))
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun my-clockfiles () (append org-agenda-files
				  (file-expand-wildcards "~/surfdrive/org/track∗.org")))
...
_How to create a new folder saved in surfdrive_
1. C-x C-f to create a new file
2. ~/ to go to home directory
3. ~/surfdrive to go to surfdrive
4. /file_name
